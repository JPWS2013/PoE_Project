...
void loop() 
{
    while (DominoCount <40)
    {
        ...
        DispenserSpeed = 0;
        DispenserDirection = 0; //Should always be either one or two, depending on motor mount orientation.
...
void RunMotors(int rs, int ls, int ds, byte rd, byte ld, byte dd) //Run two drive motors
{
    ...
    if(rs == 0 && ls == 0)
    {
        tone(SignalPin, 260); //During running, if motors are told not to move, that means that it has lost the laser.Play a low tone.
    }
    else
    {
